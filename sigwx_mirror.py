...
SIGWX_URL = (CFPS_API + "?site=CYFB&image=SIG_WX//MID_LEVEL/*")
...
def cfps_sigwx(client: httpx.Client) -> dict[str, bytes]:
    ...
    out: dict[str, bytes] = {}
    try:
        r = client.get(SIGWX_URL,
                       headers={**UA, "Accept": "application/json"},
                       follow_redirects=True)
        rows_all = r.json().get("data", []) if r.status_code == 200 else []
        if r.status_code != 200:
            print(f"  [cfps-sigwx] HTTP {r.status_code}")
    except (httpx.HTTPError, ValueError) as e:
        print(f"  [cfps-sigwx] {e}")
        rows_all = []
    if rows_all:
        print(f"  [cfps-sigwx] {len(rows_all)} Zeilen; Produkte: "
              + str([(c.get('image'), c.get('location'))
                     for c in rows_all])[:220])
    else:
        print("  [cfps-sigwx] keine Zeilen — Rueckfall auf tgftp/AWC")
        return out
...
def awc_swm_nat(client: httpx.Client) -> dict[str, bytes]:
    ...
    # Nur PGNE14: die uebrigen 14er decken fremde Regionen ab
    # (PGCE14 = Naher Osten — Befund Stefan 12.07.)
    for k, fname in enumerate(("PGNE14.PNG",), 1):
