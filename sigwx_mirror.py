...
def gfa_via_cfps(client: httpx.Client, region: str,
    ...
    out: dict[str, bytes] = {}
    try:
        r = client.get(CFPS_API, headers={**UA, "Accept": "application/json",
                                          "Referer": "https://plan.navcanada.ca/gfa/"},
                       params={"point": f"{icao}|site|{lonlat}",
                               "alpha": "gfa"},
                       follow_redirects=True)
        if r.status_code != 200:
            print(f"  [cfps] GFACN{region}: HTTP {r.status_code}")
            return out
        rows = r.json().get("data", [])
    except (httpx.HTTPError, ValueError) as e:
        print(f"  [cfps] GFACN{region}: {e}")
        return out
