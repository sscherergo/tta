...
def metar_obs_time(raw: str, now: datetime) -> datetime | None:
    """Beobachtungszeit (DDHHMMZ) in volle UTC-Zeit aufloesen, inkl.
    Monats-/Jahreswechsel. None, wenn keine Zeitgruppe gefunden."""
    ...
    for line in raw.splitlines():
        ...
        for icao in MAIN_ICAO:
            if line.startswith((icao, f"METAR {icao}", f"SPECI {icao}")):
                d = parse_metar(line)
                # echte Beobachtungszeit aus der METAR-Zeitgruppe;
                # Abrufzeit nur als Fallback (z.B. korrupte Zeitgruppe)
                t = metar_obs_time(line, now) or now
                obs.append({"time": t.isoformat(), "icao": icao,
                            **d, "raw": line[:120]})
                break
    return obs
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        new_obs = await fetch_observations(client, now)
        seen = {(o["icao"], o["time"]) for o in history["observations"]}
        history["observations"] += [o for o in new_obs
                                    if (o["icao"], o["time"]) not in seen]
