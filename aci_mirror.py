...
# Orientierung wird je Lauf SELBST kalibriert: bekannte Kuesten-
# Landmarken werden in allen 4 Kandidaten-Orientierungen projiziert;
# gewaehlt wird die mit den meisten Treffern auf roten Kuestenpixeln.
ORIENT_UP_LON = 0.0             # Startwert; calibrate() setzt beide
AZ_SIGN = 1

CAL_POINTS = [                  # markante Kuestenpunkte, alle Quadranten
    (59.8, -43.9), (76.5, -68.5), (71.4, -156.5), (64.5, -165.4),
    (66.0, -18.0), (78.0, 20.0), (77.7, 104.0), (66.0, 169.0),
    (70.0, -128.0), (68.5, -166.0), (81.5, -16.0), (73.5, 80.0),
    (58.5, -152.5), (71.0, 25.0),
]
...
def calibrate(img) -> None:
    """Orientierung (UP_LON x Spiegelung) aus den roten Kuestenlinien:
    hierarchische Suche — grob (10-Grad-Raster, grosses Fangfenster),
    dann zweistufig verfeinert. Setzt die Modulglobalen."""
    global ORIENT_UP_LON, AZ_SIGN
    w, h = img.size
    px = img.load()

    def hit(xi, yi, win, step):
        for dy in range(-win, win + 1, step):
            ...
            for dx in range(-win, win + 1, step):
                xx = xi + dx
                if 0 <= xx < w and _is_red(px[xx, yy]):
                    return True
        return False

    def score(up, sign, win, step):
        global ORIENT_UP_LON, AZ_SIGN
        ORIENT_UP_LON, AZ_SIGN = up, sign
        n = 0
        for lat, lon in CAL_POINTS:
            x, y = ll_to_px(lat, lon, w, h)
            n += hit(int(x), int(y), win, step)
        return n

    win_c = max(8, int(min(w, h) * 0.045))      # Fangfenster grob
    best, best_score = (0.0, 1), -1
    tops = []
    for up in range(-180, 180, 10):
        for sign in (1, -1):
            n = score(float(up), sign, win_c, 4)
            tops.append(((float(up), sign), n))
            if n > best_score:
                best_score, best = n, (float(up), sign)
    sign = best[1]
    # Stufe 2: 2-Grad-Raster, kleineres Fenster — eigenes Maximum
    # (Grob-Score saettigt; Tie-Break = Naehe zum Stufenzentrum)
    c2 = [(abs(d), best[0] + d) for d in range(-8, 9, 2)]
    s2 = [(score(u, sign, 8, 2), -ad, u) for ad, u in c2]
    best_score, _, u2 = max(s2)
    # Stufe 3: 0.5-Grad-Raster, enges Fenster
    c3 = [(abs(d), u2 + d) for d in
          (-1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5)]
    s3 = [(score(u, sign, 5, 1), -ad, u) for ad, u in c3]
    best_score, _, u3 = max(s3)
    best = (u3, sign)
    ORIENT_UP_LON, AZ_SIGN = best
    tag = "gespiegelt" if best[1] < 0 else "normal"
    tops.sort(key=lambda kv: -kv[1])
    print(f"[aci] Kalibrierung: UP_LON={best[0]:.1f}, {tag} "
          f"({best_score}/{len(CAL_POINTS)} Landmarken) — "
          f"Grob-Top-3: {tops[:3]}")
    if best_score < 9:
        print("[aci] WARNUNG: schwache Kalibrierung — Ausschnitt pruefen!")
