...
def calibrate(img) -> None:
    """Waehlt Orientierung (UP_LON x Spiegelung) anhand der roten
    Kuestenlinien im Bild; setzt die Modulglobalen."""
    global ORIENT_UP_LON, AZ_SIGN
    w, h = img.size
    px = img.load()
    best, best_score, scores = (0.0, 1), -1, {}
    for up in (0.0, 180.0):
        for sign in (1, -1):
                ...
                for dy in range(-4, 5):
                    ...
                    for dx in range(-4, 5):
                        if 0 <= xi + dx < w and 0 <= yi + dy < h \
                                and _is_red(px[xi + dx, yi + dy]):
                            found = True
                            break
                hits += found
            scores[(up, sign)] = hits
            if hits > best_score:
                best_score, best = hits, (up, sign)
    ORIENT_UP_LON, AZ_SIGN = best
    tag = 'gespiegelt' if best[1] < 0 else 'normal'
    print(f"[aci] Kalibrierung: UP_LON={best[0]:.0f}, {tag} "
          f"({best_score}/{len(CAL_POINTS)} Landmarken) — "
          f"Scores: {sorted(scores.items())}")
    if best_score < 6:
        print("[aci] WARNUNG: schwache Kalibrierung — "
              "Ausschnitt pruefen!")
