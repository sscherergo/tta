...
RUNWAY_TRUE: dict[str, float | None] = {
    ...
    # Alaska: FAA-NFDC "True Alignment" direkt (Cycle 2026-07-09) —
    # NICHT aus Designator+Var rechnen, mehrere Pisten wurden umnummeriert!
    "PABR": 90.0,    # RWY 08/26 (ex 07/25), FAA True Alignment 090
    "PAOM": 110.0,   # RWY 10/28, FAA True Alignment 110 (2. Piste 03/21: 042T)
    ...
    "PASC": 75.0,    # RWY 06/24 (ex 05/23), FAA True Alignment 075
    "PAWI": 67.0,    # RWY 06/24 (ex 05/23), FAA True Alignment 067 (Gravel)
    "PAPO": 36.0,    # RWY 03/21 (ex 01/19), FAA True Alignment 036
    "PAOT": 102.0,   # RWY 09/27, FAA True Alignment 102 (2. Piste 18/36: 194T Gravel)
}
...
# Hoehenabhaengige RH-Schwellen fuer die Ceiling-Ableitung. Hintergrund:
# CAPS publiziert KEIN Low-Cloud-Produkt (nur TotalCloudCover_Sfc, gesamte
# Saeule — Datamart-Verzeichnis geprueft 15.07.2026). Eine feuchte arktische
# Grenzschicht (RH 90-96% ohne Wolkendeck) unter hoeherer Bewoelkung wurde
# daher faelschlich als Ceiling ~300-400 ft interpretiert (CYRB-Muster:
# Dashboard ~400 ft bei real BKN026). Bodennah gilt deshalb eine strengere
# Saettigungsschwelle; echtes Stratus/Nebel (RH >= ~97%) triggert weiterhin.
CEILING_RH_CRIT = {"1015": 97.0, "1000": 97.0, "0985": 95.0, "0970": 93.0,
                   "0950": 92.0, "0925": 90.0, "0900": 90.0, "0875": 90.0,
                   "0850": 90.0}


def ceiling_ft(data, fh, wi) -> float | None:
    """Wolkenbasis: RH-Durchgang zwischen Druckflaechen interpoliert
    (Schwelle je Level, s. CEILING_RH_CRIT), barometrisch in ft AGL.
    Bedeckungs-Veto: TotalCloudCover < 50% kann kein BKN/OVC tragen ->
    kein Ceiling. 99999 = frei unterhalb ~5000 ft.
    ...
    for lvl in CEILING_LEVELS:                        # hoechster Druck zuerst
        ...
        if not valid(rh):
            continue
        crit = CEILING_RH_CRIT.get(lvl, 90.0)
        if rh >= crit:
            tcc = gv(data, fh, "TotalCloudCover_Sfc", wi)
            if valid(tcc) and tcc < 50.0:
                return 99999.0        # gesaettigte Flaeche, aber kein Deck
            if prev_rh is not None and prev_rh < crit and rh > prev_rh:
                f = (crit - prev_rh) / (rh - prev_rh)
...
def fog_trend(data, fh, wi) -> tuple[float | None, float | None, str]:
    ...
    if cls == "NOGO":
        cls = "WARN"            # TRD ist Projektion — der Ist-Zustand wird
        # im +6h-Schritt von SP2m selbst bewertet; eine Vorhersage der
        # Vorhersage loest kein Gesamt-NOGO mehr aus (FAR-Treiber).
    return d6, proj, cls
...
def dashboard_block(data, date, run, waypoints, obs=None) -> list[str]:
    ...
    lines = [
        ...
        "(Klasse = SP-Schwellen auf die Projektion, max. WARN; negativ = "
        "schliessend) | "
